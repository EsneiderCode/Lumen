...
## Working with upstream — MANDATORY PROCESS

This project lives across three remotes. **Read this section before any code change.**

### Remote topology

| Remote | URL | Role |
|---|---|---|
| `upstream` | `EsneiderCode/Lumen` | **Source of truth.** Alejandro merges all PRs here. Has `main` and `develop`. |
| `origin`   | `HMR-Nexus/Lumen`    | Organization mirror. May lag behind upstream. |
| `fork`     | `jarl9801/Lumen`     | Jarl's personal fork. **Push feature branches here**, then open cross-fork PRs. |

### Mandatory flow per feature

1. **`npm run feature:start feat/<thing>`** — fetches all remotes, prunes zombie branches, fast-forwards local main if safe, creates the new branch from `upstream/develop`, runs baseline tests, prints the next migration number. **Always start here.**
2. Make the change. Add tests. Update specs.
3. **`npm run feature:check`** — validates rebase target, tests, typecheck, migration numbering, no committed `.env`, no missing dependency declarations.
4. `git push -u fork feat/<thing>`
5. `gh pr create --repo EsneiderCode/Lumen --base develop --head jarl9801:feat/<thing>` — PRs **always target `upstream/develop`**, never `main` directly. Alejandro merges develop → main on his cadence.

### Hard rules

- **Migrations**: numbered sequentially. Always check `git ls-tree upstream/develop supabase/migrations/` before picking a number. New migrations MUST declare `Depends on:` in their header.
- **Supabase apply**: never apply migrations from this machine — Alejandro applies them in his Supabase. We only ship the `.sql`.
- **`database.types.ts`**: regenerated by Alejandro after he applies the migration. Do not edit by hand.
- **Branches**: `feat/* | fix/* | chore/* | docs/* | refactor/* | test/*` + kebab-case. Anything else is rejected by `feature:check`.
