...
-- ── 2. Fusion details: cabinet identifier + card count for POP ───────────
...
-- ── 3. Billing lines: snapshot pricing at certification time ─────────────
-- A work order can have multiple billable items (Alta is the main case).
-- unit_price_snapshot decouples historical billings from later catalog edits.
...
-- ── 4. State machine: direct-order shortcut ──────────────────────────────
-- Direct orders (client_id IS NULL) jump from internally_certified straight
-- to invoiced — there is no client to send to or accept from.
-- Mirror this in src/services/workOrderService.ts VALID_TRANSITIONS.
...
BEGIN
  ...
  IF NOT (
    ...
    -- 'paid' and 'cancelled' have no valid outbound transitions
  ) THEN
    RAISE EXCEPTION
...
END;
$$;
-- Trigger trg_validate_wo_status_transition is already attached to work_orders
-- by rls_policies.sql; replacing the function above is enough.
